...
def main():
    ...
    st.write("1. Upload an image using the file uploader on the left sidebar.")
    st.write("2. Adjust the parameters for face detection using the sliders below.")
    st.write("3. Choose the color for the rectangles drawn around the detected faces using the color picker.")
    ...
    if uploaded_file is not None:
        ...
        if st.button("Detect Faces"):
            ...
            st.image(image_with_faces, channels="BGR", caption="Image with Detected Faces", use_column_width=True)
